...
スポット一覧・検索クエリの実行計画を分析するための PostgreSQL コンテナ。
...
- **`randomPastDate` は日オフセット × 生成時刻** — `new Date(Date.now() - randomInt(0, 180) * 86400000)`。日オフセットは181通りだが `Date.now()` が生成のたびに進むため、実際の値はもっと散る（3,000件で実測1,869通り／同値あたり平均1.6件）。それでも重複は残るので**カーソルのタイブレーク欠落（H4）は再現する**。意図的にこのままにしている
